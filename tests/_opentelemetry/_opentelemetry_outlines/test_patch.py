...
class MockModel:
    ...
    def generate(
        self, prompts, generation_parameters, logits_processor, sampling_parameters
    ):
        return "sync result"
...
def test_model_generate_error(mock_tracer, mock_span):
    ...
    with pytest.raises(Exception) as exc_info:
        decorated_error(
            mock_model.error_generate, mock_model, ("prompt", gp, lp, sp), {}
        )
...
async def test_model_generate_async(mock_tracer, mock_span):
    ...
    result = await decorated(
        mock_model.generate_chat, mock_model, ("prompt", gp, lp, sp, None), {}
    )
...
async def test_model_generate_async_error(mock_tracer, mock_span):
    ...
    mock_model = ErrorMockModel()
    with pytest.raises(Exception) as exc_info:
        await decorated_error(
            mock_model.error_async, mock_model, ("prompt", gp, lp, sp, None), {}
        )
